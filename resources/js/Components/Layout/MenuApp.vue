<script setup>
  ...
  const enderecos = [
    ...
    { name: 'Bairros', description: 'Realize o cadastro / edição de bairros.', href: '/CadBairro', icon: MapIcon },

  ]

  const clientes = [
    { name: 'Lista de Clientes', description: 'Realize o cadastro / edição de novos clientes.', href: '/Clientes', icon: UserIcon   },
  ]
  ...
  const paineis = [
    ...
    { name: 'Gerar PI', description: 'Gera PI das reservas na Bi-Semana selecionada.', href: '/ReservaSemPi', icon: PhotoIcon },

  ]

  const vendas = [
    { name: 'Lançar Venda', description: 'Lançar venda de serviços sem reserva.', href: '/Vendas/Lancar', icon: CurrencyDollarIcon }
  ]
  ...
  const financeiro = [
    ...
    { name: 'Comissões Pagas', description: 'Lista as comissões pagas por PI.', href: '/ListaComissoesPagas', icon: CurrencyDollarIcon },
        ...
        <PopoverGroup class="hidden lg:flex lg:gap-x-4">
          ...
          <Popover class="relative">
            ...
            <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
              <PopoverPanel class="absolute -left-8 top-full z-10 mt-3 w-screen max-w-md max-h-[65vh] overflow-auto rounded-3xl bg-white shadow-lg ring-1 ring-gray-900/5 over">
                <div class="p-4">
                  <div v-for="item in vendas" :key="item.name" class="group relative flex items-center gap-x-6 rounded-lg p-4 text-sm leading-6 hover:bg-gray-50">
                    <div class="flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
                      <component :is="item.icon" class="h-6 w-6 text-gray-600 group-hover:text-indigo-600" aria-hidden="true" />
                    </div>
                    <div class="flex-auto">
