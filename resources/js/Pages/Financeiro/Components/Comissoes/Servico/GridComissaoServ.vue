...
<script setup>
    ...
    import EditComissaoServ from './EditComissaoServ.vue';
    import DelComissaoServ from './DelComissaoServ.vue';
...
<template>
    ...
    <div class="card w-full h-[42rem] bg-base-100 border border-base-200 shadow-xl overflow-auto rounded-md">
        <div class="card-body flex flex-col sm:flex-row">
                                ...
                                <div class="w-full">
                                    <div class="sm:flex flex-wrap">   
                                        <div class="w-full flex flex-col items-center space-y-3 mt-4">
                                            ...
                                            <p class="text-xs sm:text-lg">Comissão do Serviço:</p>

                                            <p v-if="serv.tipo_comissao == 0" class="text-xs sm:text-xl font-extrabold hover:text-red-700 sm:ml-4">
                                                {{cashBR(serv.comissao)}}     
                                            </p>

                                            <p v-else class="text-xs sm:text-xl font-extrabold hover:text-red-700 sm:ml-4">
                                                {{(serv.comissao)}} %    
                                            </p>
                                        ...
                                        <div class="w-full flex items-center justify-around mt-4 space-x-2">
                                            <label class="inline-flex w-6/12 justify-center rounded-md bg-green-500 px-3 py-2 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-green-300 sm:mt-0 sm:w-5/12" @click="openEdit('t', serv)">Incluir Comissão</label>
                                            <label class="inline-flex w-6/12 justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-300 sm:ml-3 sm:w-5/12" @click="openDel('t', serv)">Excluir Comissão</label>
                                        </div>
                                    </div>
                                </div>
            ...
            <EditComissaoServ :openEdit="openE" @closeEdit="openEdit" :servico="servicoSelecionado">
            </EditComissaoServ>

            <DelComissaoServ :openDel="openD" @closeDel="openDel" :servico="servicoSelecionado">
            </DelComissaoServ>
        </div>
    </div>
