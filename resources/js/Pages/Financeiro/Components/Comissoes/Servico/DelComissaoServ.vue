...
const props = defineProps(['openDel', 'servico']);
...
function delComissao(val) {
    axios.post('/DelComissaoServico', {id_servico: val})
    .then((res) => {
        toastr.success('Comissão excluida com sucesso!')
        setTimeout(() => {
          openDel('f')
                ...
                <div>   
                  ...
                  <div class="mt-3 text-center sm:mt-5">
                    <DialogTitle as="h3" class="text-xl font-semibold leading-6 text-gray-900">
                        Excluir Comissão: <span class="text-red-500 font-extrabold">{{props.servico.nome}}</span>
                    </DialogTitle>
                    
                    <div class="mt-2">
                      <p class="text-lg text-gray-500">
                        Você tem certeza que deseja excluir a comissão deste serviço?
                      </p>
                    </div>
                    <div class="mt-5 sm:mt-6 w-full space-y-4 border-t border-gray-200 pt-4 sm:space-x-8">
                        <label id="btnSendServ" class="inline-flex w-10/12 justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-5/12" @click="delComissao(props.servico.id)">Excluir</label>
